...
impl S3Store {
    ...
    /// Uploads every file under `dir` to `<prefix>/<path relative to dir>`,
    /// like `aws s3 cp --recursive`.
    pub async fn upload_dir(&self, dir: &std::path::Path, prefix: &str) -> Result<()> {
        ...
        for file in files {
            ...
            self.write(&key, body, content_type_for(&file)).await?;
        }
        Ok(())
    }
...
/// A content type for an uploaded file, matching what the AWS CLI guesses.
pub fn content_type_for(path: &std::path::Path) -> &'static str {
        ...
        .as_deref()
    {
        ...
        Some("sigstore") => "application/json",
