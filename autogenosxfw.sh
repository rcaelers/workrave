...
export PATH=$HOME/Source/gtkfw/inst/bin/:$PATH
export PERL5LIB=$PERL5LIB/:$HOME/Source/gtkfw/inst/lib/perl5/vendor_perl:$HOME/Source/gtkfw/inst/lib/perl5/site_perl

for p in $HOME/src/gtk-osx-framework/framework/Frameworks/*
do
  if [ -d $p/Resources/dev/lib/pkgconfig ]; then
      if [ -z $PKG_CONFIG_PATH ]; then
          export PKG_CONFIG_PATH=$p/Resources/dev/lib/pkgconfig
      else
          export PKG_CONFIG_PATH=$PKG_CONFIG_PATH:$p/Resources/dev/lib/pkgconfig
      fi
  fi
done
