// X11AppletWindow.cc --- Applet info Window
//
 ...
 */
X11AppletWindow::X11AppletWindow() :
  timer_box_view(NULL),
...
void
X11AppletWindow::init()
{
  TRACE_ENTER("X11AppletWindow::init");
...
void
X11AppletWindow::init_applet()
{
  TRACE_ENTER("X11AppletWindow::init_applet");
...
//! Destroys the applet.
void
X11AppletWindow::destroy_applet()
{
  TRACE_ENTER("X11AppletWindow::destroy_applet");
...
bool
X11AppletWindow::init_tray_applet()
{
  TRACE_ENTER("X11AppletWindow::init_tray_applet");
  bool ret = false;
  ...
  if (tray_icon != NULL)
    {
      ...
      eventbox->signal_button_press_event().connect(MEMBER_SLOT(*this, &X11AppletWindow::on_button_press_event));
      container = eventbox;
      ...
      plug->signal_embedded().connect(MEMBER_SLOT(*this, &X11AppletWindow::on_embedded));
      plug->signal_delete_event().connect(MEMBER_SLOT(*this, &X11AppletWindow::delete_event));
...
void
X11AppletWindow::destroy_tray_applet()
{
  if (mode == APPLET_TRAY)
    {
      set_mainwindow_applet_active(false);
...
bool
X11AppletWindow::init_gnome_applet()
{
  TRACE_ENTER("X11AppletWindow::init_gnome_applet");
  bool ok = true;
  ...
  if (ok)
    {
      ...
      g_signal_connect(G_OBJECT(plug->gobj()), "destroy-event",
                       G_CALLBACK(X11AppletWindow::destroy_event), this);
      ...
      plug->signal_button_press_event().connect(MEMBER_SLOT(*this, &X11AppletWindow::on_button_press_event));
      plug->signal_button_release_event().connect(MEMBER_SLOT(*this, &X11AppletWindow::on_button_press_event));
...
void
X11AppletWindow::destroy_gnome_applet()
{
...
bool
X11AppletWindow::init_kde_applet()
{
  TRACE_ENTER("X11AppletWindow::init_kde_applet");
...
//! Destroys the native gnome applet.
void
X11AppletWindow::destroy_kde_applet()
{
  if (mode == APPLET_KDE)
    {
      // Cleanup Widgets.
...
bool
X11AppletWindow::delete_event(GdkEventAny *event)
{
  (void) event;
  set_mainwindow_applet_active(false);
  destroy_applet();
...
void
X11AppletWindow::fire_gnome_applet()
{
  if (mode == APPLET_TRAY)
    {
      destroy_tray_applet();
...
void
X11AppletWindow::fire_kde_applet()
{
  if (mode == APPLET_TRAY)
    {
      destroy_tray_applet();
...
void
X11AppletWindow::set_applet_control(GNOME_Workrave_AppletControl applet_control)
{
  if (mode == APPLET_TRAY)
    {
      destroy_tray_applet();
...
void
X11AppletWindow::update()
{
  TRACE_ENTER("X11AppletWindow::update");
...
void
X11AppletWindow::set_menu_active(int menu, bool active)
{
  CORBA_Environment ev;
...
bool
X11AppletWindow::get_menu_active(int menu)
{
  CORBA_Environment ev;
  bool ret = false;
...
void
X11AppletWindow::set_applet_vertical(bool v)
{
  TRACE_ENTER_MSG("X11AppletWindow::set_applet_vertical", applet_vertical);
...
void
X11AppletWindow::set_applet_size(int size)
{
  TRACE_ENTER_MSG("X11AppletWindow::set_applet_size", size);
...
//! Sets the size of the applet.
void
X11AppletWindow::set_applet_background(int type, GdkColor &color, long xid)
{
  TRACE_ENTER_MSG("X11AppletWindow::set_applet_pixmap", xid);
...
gboolean
X11AppletWindow::destroy_event(GtkWidget *widget, GdkEvent *event, gpointer user_data)
{
  (void) event;
  (void) widget;
  if (user_data != NULL)
    {
      X11AppletWindow *applet = (X11AppletWindow *) user_data;
...
bool
X11AppletWindow::on_button_press_event(GdkEventButton *event)
{
  bool ret = false;
...
void
X11AppletWindow::button_clicked(int button)
{
  (void) button;
...
X11AppletWindow::AppletMode
X11AppletWindow::get_applet_mode() const
{
  return mode;
}
...
void
X11AppletWindow::read_configuration()
{
...
void
X11AppletWindow::config_changed_notify(string key)
{
  TRACE_ENTER_MSG("X11AppletWindow::config_changed_notify", key);
  (void) key;
  read_configuration();
  TRACE_EXIT();
}
...
void
X11AppletWindow::on_embedded()
{
  TRACE_ENTER("X11AppletWindow::on_embedded");
  set_mainwindow_applet_active(true);
...
void
X11AppletWindow::set_mainwindow_applet_active(bool a)
{
  TRACE_ENTER_MSG("X11AppletWindow::set_mainwindow_applet_active", a);
  GUI *gui = GUI::get_instance();
