...
static void wrgtk_tray_icon_style_set(GtkWidget *widget, GtkStyle *previous_style);
static gboolean wrgtk_tray_icon_delete(GtkWidget *widget, GdkEventAny *event);
static gboolean wrgtk_tray_icon_expose(GtkWidget *widget, GdkEventExpose *event);
...
{
  icon->priv = G_TYPE_INSTANCE_GET_PRIVATE(icon, GTK_TYPE_TRAY_ICON, WRGtkTrayIconPrivate);
...
static gboolean
wrgtk_tray_icon_expose(GtkWidget *widget, GdkEventExpose *event)
{
  ...
  if (focus_child && GTK_WIDGET_HAS_FOCUS(focus_child))
    {
      ...
      gtk_paint_focus(
        widget->style, widget->window, GTK_WIDGET_STATE(widget), &event->area, widget, "tray_icon", x, y, width, height);
    }
...
wrgtk_tray_icon_send_dock_request(WRGtkTrayIcon *icon)
{
  ...
  wrgtk_tray_icon_send_manager_message(
    icon, SYSTEM_TRAY_REQUEST_DOCK, icon->priv->manager_window, gtk_plug_get_id(GTK_PLUG(icon)), 0, 0);
}
...
wrgtk_tray_icon_update_manager_window(WRGtkTrayIcon *icon)
{
  ...
  if (icon->priv->manager_window != None)
    ...
    GTK_NOTE(PLUGSOCKET, g_print("GtkStatusIcon %p: no tray manager found\n", icon));
}
...
wrgtk_tray_icon_realize(GtkWidget *widget)
{
  ...
  GTK_NOTE(PLUGSOCKET,
           g_print("GtkStatusIcon %p: realized, window: %lx, socket window: %lx\n",
                   ...
                   GTK_PLUG(icon)->socket_window ? (gulong)GDK_WINDOW_XWINDOW(GTK_PLUG(icon)->socket_window) : 0UL));
...
static void
wrgtk_tray_icon_style_set(GtkWidget *widget, GtkStyle *previous_style)
{
...
WRGtkTrayIcon *
wrgtk_tray_icon_new_for_screen(GdkScreen *screen, const gchar *name)
{
  g_return_val_if_fail(GDK_IS_SCREEN(screen), NULL);

  return g_object_new(GTK_TYPE_TRAY_ICON, "screen", screen, "title", name, NULL);
}

WRGtkTrayIcon *
wrgtk_tray_icon_new(const gchar *name)
{
  return g_object_new(GTK_TYPE_TRAY_ICON, "title", name, NULL);
}
