    ...
    int code, WPARAM wpar, LPARAM lpar, HHOOK hook, BOOL forcecallnext )
{
  ...
  if(is_app_blocked() && !forcecallnext &&
      block_input && code == HC_ACTION )
      // block mouse input:
      return (LRESULT) -1;
  else
      ...
      if( menu && menu_popup )
        {
          ...
          AppendMenu( menu_popup, MF_STRING | MF_CHECKED, IDM_MENU_MONITOR,
...
harpoon_hook( HarpoonHookFunc func, BOOL keyboard_lowlevel, BOOL mouse_lowlevel )
{
  ...
  */
  ...
  if( mouse_lowlevel == TRUE )
    {
      mouse_ll_hook = SetWindowsHookEx(
          WH_MOUSE_LL, harpoon_mouse_ll_hook, dll_handle, 0 );
      if( mouse_ll_hook )
  ...
  */
  mouse_hook = SetWindowsHookEx(
      WH_MOUSE, harpoon_mouse_hook, dll_handle, 0 );
  if( mouse_hook )
  ...
  if( keyboard_lowlevel == TRUE )
    {
      keyboard_ll_hook = SetWindowsHookEx(
          WH_KEYBOARD_LL, harpoon_keyboard_ll_hook, dll_handle, 0 );
      if( keyboard_ll_hook )
...
static void debug_send_message( const char *str )
 /*
...
*/
{
  ...
  */
  ...
  if( local.wSecond == wSecond_previous_call &&
      local.wMinute == wMinute_previous_call &&
      local.wHour == wHour_previous_call &&
  ...
  snprintf( buffer, 1023, "%lu:%02lu:%02lu %s:\t%s (%lu):\t %s\r\n",
      (DWORD)local.wHour, (DWORD)local.wMinute, (DWORD)local.wSecond,
          mer, exec_filename, exec_process_id, str );
...
static void debug_process_menu_selection( WORD idm )
{
  DWORD ret;
...
// This is called if the user opts to save the messages to a file.
{
  ...
  GetSaveFileNameA = ( BOOL ( WINAPI * ) ( OPENFILENAME * ) )
      GetProcAddress( hmm, "GetSaveFileNameA" );
