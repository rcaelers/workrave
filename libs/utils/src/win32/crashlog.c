...
static EXCEPTION_DISPOSITION __cdecl double_exception_handler(struct _EXCEPTION_RECORD *exception_record,
...
LONG WINAPI
exception_filter(EXCEPTION_POINTERS *ep)
{
  return exception_handler(ep->ExceptionRecord, NULL, ep->ContextRecord, NULL);
}

EXCEPTION_DISPOSITION __cdecl exception_handler(struct _EXCEPTION_RECORD *exception_record,
                                                void *establisher_frame,
                                                struct _CONTEXT *context_record,
                                                void *dispatcher_context)
{
  ...
  */
  ...
  fprintf(log,
          "Crash log created on %02d/%02d/%04d at %02d:%02d:%02d.\n\n",
          SystemTime.wDay,
          SystemTime.wMonth,
          SystemTime.wYear,
          SystemTime.wHour,
  ...
  if (GetEnvironmentVariableW && CreateFileW)
    // >= WinNT
    {
      WCHAR *one = L"Workrave has unexpectedly crashed. A crash log has been saved to:\n";

      WCHAR *two =
        L"\nPlease file a bug report: http://issues.workrave.org/\n"
        L"Thanks.";

      WCHAR *nomem = L"Workrave is out of memory!";
...
static BOOL WINAPI
stack_walk(HANDLE process, LPSTACKFRAME stack_frame, PCONTEXT context_record)
{
  if (!stack_frame->Reserved[0])
  ...
  else
    {
      ...
      if (!ReadProcessMemory(process, (LPCVOID)stack_frame->AddrFrame.Offset, &stack_frame->AddrFrame.Offset, sizeof(DWORD), NULL))
        return FALSE;
  ...
  while (TRUE)
    {
      ...
      fprintf(log, "%08X  %08X  %08X\n", (int)sf.AddrPC.Offset, (int)sf.AddrFrame.Offset, (int)sf.AddrReturn.Offset);
    }
}
...
print_module_info(FILE *log, HMODULE mod)
{
  ...
  fprintf(log,
          " %-20s handle: %p date: %d-%.2d-%.2d %.2d:%.2d:%.2d\n",
          buffer,
          mod,
          file_time.wYear,
          file_time.wMonth,
...
save_key(FILE *log, HKEY key, char *name)
{
  ...
  for (i = 0;; i++)
    {
      HKEY subkey;
      LONG rc = RegEnumKey(key, i, keyname + keyname_len, sizeof(keyname) - keyname_len);
