...
#include "Break.hh"
#include "Core.hh"
#include "RpcDBusLegacyBreakBindingDBus.hh"
#include "RpcDBusLegacyConfigBindingDBus.hh"
...
namespace generated = workrave::core::legacy_rpc;
...
struct LegacyRpcDBusServer::Impl
{
    ...
    for (workrave::BreakId id = workrave::BREAK_ID_MICRO_BREAK; id < workrave::BREAK_ID_SIZEOF; id++)
      {
        ...
        const std::string break_path =
          std::string(dbus_root_path) + "/Break/" + break_controller->get_name();
        auto binding =
...
LegacyRpcDBusServer::LegacyRpcDBusServer(Core &core, workrave::config::IConfigurator &configurator)
  : impl_(std::make_unique<Impl>(core, configurator))
{
}

LegacyRpcDBusServer::~LegacyRpcDBusServer() = default;
